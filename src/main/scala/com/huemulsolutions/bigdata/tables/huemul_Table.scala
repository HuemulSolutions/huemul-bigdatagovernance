...
class huemul_Table(huemulBigDataGov: huemul_BigDataGovernance, Control: huemul_Control) extends huemul_TableDQ with Serializable  {
  ...
  */
    ...
    else if (_TableType == huemulType_Tables.Reference || _TableType == huemulType_Tables.Master)
    {
      ...
      if (this.getStorageType == huemulType_StorageType.HBASE) {
        ...
        if (hBaseConnector.tableExistsHBase(getHBaseNamespace(huemulType_InternalTableType.Normal),getHBaseTableName(huemulType_InternalTableType.Normal))) {
          ...
          //new from 2.3
          runSQLexternalTable(huemulType_InternalTableType.Normal, CreateTableScript, true)
    ...
    if (Result) {
      ...
      try {
        ...
        if (getStorageType == huemulType_StorageType.HBASE) {          
          ...
          //new from 2.3
          runSQLexternalTable(huemulType_InternalTableType.Normal, CreateTableScript, false)
        }
        ...
        if (_PartitionField != null && _PartitionField != "") {
          ...
          //new from 2.3
          runSQLexternalTable(huemulType_InternalTableType.Normal, _refreshTable, false)          
        }
  ...
  private def savePersist_OldValueTrace(LocalControl: huemul_Control, DF: DataFrame): Boolean = {
    ...
    if (Result) {
      ...
      try {
        ...
        //new from 2.3
        runSQLexternalTable(huemulType_InternalTableType.OldValueTrace, lscript, false)        
        ...
        //new from 2.3
        runSQLexternalTable(huemulType_InternalTableType.OldValueTrace, _refreshTable, false)        
  ...
  def savePersist_DQ(LocalControl: huemul_Control, DF: DataFrame): Boolean = {
    ...
    if (Result) {
      ...
      try {
        ...
        //new from 2.3
        runSQLexternalTable(huemulType_InternalTableType.DQ, lscript, false)       
        ...
        if (huemulBigDataGov.DebugMode) huemulBigDataGov.logMessageDebug(_refreshTable)
        //new from 2.3
        runSQLexternalTable(huemulType_InternalTableType.DQ, _refreshTable, false)        
  ...
  private def runSQL_dropExternalTable(tableType: huemulType_InternalTableType, databaseName:String, tableName:String, onlyHBASE: Boolean) {
    ...
    //if (tableType == huemulType_InternalTableType.Normal) {
      if (onlyHBASE) {
  ...
  private def runSQLexternalTable(tableType: huemulType_InternalTableType, sqlSentence:String, onlyHBASE: Boolean) {
    //new from 2.3
    //if (tableType == huemulType_InternalTableType.Normal) {
      if (onlyHBASE) {
        /**** D R O P   F O R   H B A S E ******/
